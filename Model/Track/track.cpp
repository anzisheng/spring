...
#include <locale>
#endif
Track::Track(const std::wstring& sFilePath, const std::wstring& sTrackName, FMOD::System* pSystem)
//Track::Track(const std::wstring& sFilePath, const std::wstring& sTrackName, MainWindow *pMainWindow, FMOD::System* pSystem)
{
	...
	isPaused = false; //modify by eagle for logic. ths first time pasue 
...
//add by eagle for new define about pasue 
bool Track::pauseHandleTrack(bool setstatus)
{
	FMOD_RESULT result;
	bool bPausedTrack;
	...
	if (!isPaused) {
		...
		// Resume the track from the paused position
